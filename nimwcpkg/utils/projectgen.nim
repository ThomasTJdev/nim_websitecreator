...
template pluginSkeleton*() =
  ...
  creates "", pluginName, authorName, authorMail, authorWeb, folder, ext
  styledEcho(fgCyan, bgBlack, skeletonMsg)
  pluginName[] = readLineFromStdin("Plugin name: ").strip.normalize
  ...
  if readLineFromStdin("Generate optional compile-time config files? (y/N): ") == "y":
    if readLineFromStdin("Use NimScript instead of CFG? (y/N): ") == "y":
  ...
  deallocs pluginName, authorName, authorMail, authorWeb, folder, ext
